...
def _looks_incomplete(text: str) -> bool:
    ...
    if re.search(r"\b(if|or|and|because|but|so|when|while|that|which|who)\s*$", clean, flags=re.IGNORECASE):
        return True
...
def _trim_output(text: str, max_chars: int) -> str:
    ...
    candidate = clean[:max_chars].rstrip()

    sentence_safe = _last_complete_sentence(candidate)
    if sentence_safe and len(sentence_safe) >= 32:
        return sentence_safe

    paragraph_chunks = [part.strip() for part in re.split(r"\n{2,}", candidate) if part.strip()]
    if paragraph_chunks:
        rebuilt = "\n\n".join(paragraph_chunks).strip()
        sentence_safe = _last_complete_sentence(rebuilt)
        if sentence_safe and len(sentence_safe) >= 24:
            return sentence_safe

    fallback = candidate.rsplit(" ", 1)[0].strip()
    sentence_safe = _last_complete_sentence(fallback)
    if sentence_safe:
        return sentence_safe

    return fallback if fallback else candidate
...
def run_chat_turn(
    ...
    if _looks_incomplete(final_reply):
        repaired = _last_complete_sentence(draft_reply)
        if repaired:
